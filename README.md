...
Node >=22.13.0. Run `npm ci`, `npm run setup:local`, then `npm run dev`. Open `/login` and use the password written to `.local-login.txt` on fresh setup. `npm run build` produces the Cloudflare Worker bundle. Production: `npm run db:migrate`, then `npm run deploy`. Set runtime secrets in Cloudflare or through Wrangler; never commit `.env`, `.secrets.production.json` or password files.

## Persistent state

D1 stores projects, settings, schedules, encrypted social credentials and login rate limits. This standalone deployment uses Cloudflare KV `MEDIA_KV` for immutable images because R2 is not enabled on the owner's account. The adapter also supports an R2 `MEDIA` binding when available. KV has eventual consistency: newly uploaded images may take 60 seconds or more to appear in another region. The application is a single-owner workspace protected at the Worker boundary by a password login and HMAC-signed HttpOnly sessions. Client-supplied identity headers are discarded; API and private asset routes require a verified session. Native cron and HMAC-signed, expiring JPEG URLs are the only intentional exceptions. This is not a public multi-tenant SaaS.
